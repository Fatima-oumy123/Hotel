...
<head>
    ...
    <title>{{ config('hotel.name', 'Keur Ndiaye Lo') }} - Luxe familial a Rufisque</title>
        ...
        <div class="container">
            ...
            <div class="hero-grid" id="accueil">
                <div class="hero-copy">
                    ...
                    <p>
                        Bienvenue a <strong>{{ config('hotel.name', 'Keur Ndiaye Lo') }}</strong> : un hotel familial et elegant a Rufisque,
                        dans une zone calme, non loin du Lac Rose. Confort, prix abordables et environnement agreable pour toute la famille.
                    </p>

                    <div class="cta-row">
                        <a href="{{ route('visitor.portal') }}" class="btn">Visiter</a>
                        <a href="{{ route('guest.step1') }}" class="btn-outline">Reserver</a>
                    </div>
                    ...
                    <div class="quick-book">
                        ...
                        <form action="{{ route('guest.step1') }}" method="GET">
                            <div class="booking-grid">
                                <div class="field">
                                    <label>Arrivee</label>
                                    <input type="date" name="check_in" value="{{ now()->addDay()->toDateString() }}" min="{{ now()->toDateString() }}" required>
                                </div>
                                <div class="field">
                                    <label>Depart</label>
                                    <input type="date" name="check_out" value="{{ now()->addDays(2)->toDateString() }}" min="{{ now()->addDay()->toDateString() }}" required>
                                </div>
                                <div class="field">
        ...
        <div class="container">
            ...
            <div class="rooms-grid">
                @forelse($featuredRooms as $index => $room)
                    ...
                    @endphp
                    <article class="room">
                        <div class="room-photo" style="background-image:url('{{ $images[$index % count($images)] }}')"></div>
                        <div class="room-body">
                            <h3>{{ $room->roomType->name ?? 'Chambre' }}</h3>
                            <div class="room-meta">Chambre {{ $room->number }} · Statut {{ ucfirst($room->status) }}</div>
                            <div class="room-price">Disponible a partir de {{ number_format((float) ($room->base_price ?? 0), 0, ',', ' ') }} {{ config('hotel.currency') }}</div>
                        </div>
                    </article>
                @empty
        ...
        <div class="container split">
            ...
            <div class="panel">
                ...
                <div class="cta-row" style="margin-top:24px">
                    <a href="{{ route('visitor.portal') }}" class="btn">Voir les chambres</a>
                    <a href="{{ route('login') }}" class="btn-outline" style="color:#171717;border-color:#d4c7b9">Espace staff</a>
                </div>
            </div>
        </div>
    </section>
    ...
    <footer class="footer" id="contact">
        <div class="container">
            <div class="footer-grid">
                <div>
                    <h4>{{ config('hotel.name', 'Keur Ndiaye Lo') }}</h4>
                    <p>{{ config('hotel.address', 'DAKAR, Keur Ndiaye Lo') }}</p>
                    <p>{{ config('hotel.phone', '+221 33 662 63 93') }}</p>
                    <p>{{ config('hotel.email', 'contact@hotel.com') }}</p>
                </div>
                <div>
                    <h4>Parcours</h4>
                    <a href="{{ route('visitor.portal') }}">Visiter l hotel</a>
                    <a href="{{ route('guest.step1') }}">Reserver une chambre</a>
            ...
            <div class="copy">© {{ date('Y') }} {{ strtoupper(config('hotel.name', 'Mon Hotel')) }}. Plateforme hoteliere moderne adaptee à tous.</div>
        </div>
    </footer>
</body>
</html>
