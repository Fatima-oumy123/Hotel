...
@section('page_title', 'Console interne')
@section('page_subtitle', 'Pilotage quotidien de l hotel')
...
<div class="dashboard-page">
    <div class="dashboard-head">
        <div class="dashboard-title">
            ...
            <p>Vue d'ensemble de l'activite du {{ now()->translatedFormat('d M Y') }}</p>
        </div>
        <div class="dashboard-tools">
            @if($isAdminLike)
                <a href="{{ route('reports.export', ['month' => now()->month, 'year' => now()->year, 'type' => 'csv']) }}" class="btn-secondary">Exporter</a>
            @endif
            <a href="{{ route('reservations.index') }}" class="btn-secondary" style="background:#5d5f63;border-color:#5d5f63">Filtrer</a>
        </div>
    </div>
    ...
    <div class="dash-kpis">
        <article class="dash-kpi">
            ...
            <div class="value">{{ number_format($stats['occupancy_rate'], 0, ',', ' ') }}%</div>
            <div class="meta success">↗ {{ max(1, round($stats['occupancy_rate'] / 16, 1)) }}% vs hier</div>
        </article>

        <article class="dash-kpi">
            <div class="label">Revenus du jour ({{ config('hotel.currency', 'XOF') }})</div>
            <div class="value">{{ number_format($stats['revenue_today'], 0, ',', ' ') }}</div>
        ...
        <article class="dash-kpi">
            <div class="label">{{ $isHr ? 'Equipe active' : 'Arrivees prevues' }}</div>
            <div class="value">{{ $isHr ? ($stats['total_rooms'] ?? 0) : $stats['arrivals_today'] }}</div>
            <div class="meta">⦿ {{ $stats['pending_reservations'] }} deja enregistrees</div>
        </article>

        <article class="dash-kpi">
            <div class="label">{{ $isHr ? 'Taches RH' : 'Taches en attente' }}</div>
            <div class="value" style="color:#a55a00">{{ $stats['pending_reservations'] }}</div>
            <div class="meta danger">! {{ max(1, $stats['pending_reservations']) }} urgentes</div>
        </article>
    </div>
    ...
    <div class="dash-main">
        <section class="dash-panel">
            <div class="dash-panel-head">
                <h3>{{ $isReceptionist ? 'Flux d activite hebdomadaire' : 'Flux de Revenus Hebdomadaire' }}</h3>
                ...
                <div class="revenue-wrap">
                    @foreach($weeklyChart as $entry)
                        ...
                        <div class="revenue-col">
                            <div class="revenue-bar {{ $active ? 'active' : '' }}" style="height:{{ $height }}%">
                                <span class="revenue-day">{{ \Carbon\Carbon::createFromFormat('d/m', $entry['date'])->locale('fr')->isoFormat('ddd') }}</span>
                            </div>
                        </div>
                    @endforeach
                </div>
        ...
        <section class="dash-panel ussd-box">
            <div class="dash-panel-body">
                ...
                <div style="margin-top:16px">
                    <label class="form-label">Montant ({{ config('hotel.currency', 'XOF') }})</label>
    ...
    <section class="dash-panel">
        <div class="dash-panel-head">
            ...
            <a href="{{ route('reservations.index') }}" class="link-accent">Voir tout</a>
        </div>
                        ...
                        <tr>
                            <td>
                                <div class="guest-line">
                                    ...
                                    <small>ID: {{ $reservation->booking_number }}</small>
                                </div>
                            </td>
                            <td>
                                <div class="room-line">
                                    <span>◫</span>
                                    <span>{{ $reservation->room?->roomType?->name ?? 'Chambre' }} {{ $reservation->room?->number }}</span>
                                </div>
                            </td>
                            <td>{{ $reservation->check_in?->translatedFormat('d M') }} - {{ $reservation->check_out?->translatedFormat('d M') }}</td>
                            <td>
                                @php
                                ...
                                @endphp
                                <span class="{{ $badgeClass }}">{{ strtoupper($reservation->status_label) }}</span>
                            </td>
                            <td>{{ number_format($reservation->final_amount, 0, ',', ' ') }} {{ config('hotel.currency', 'XOF') }}</td>
        ...
        <section class="dash-panel">
            ...
            <div class="dash-panel-body">
                ...
                <div class="room-legend">
                    <span><i style="background:#22c55e"></i> Libre ({{ $stats['available_rooms'] }})</span>
                    <span><i style="background:#a55a00"></i> Occupe ({{ $stats['occupied_rooms'] }})</span>
                    <span><i style="background:#6b7280"></i> Menage ({{ $stats['maintenance_rooms'] }})</span>
                </div>
            </div>
        </section>
